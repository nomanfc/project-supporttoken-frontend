...
import { get_all_services } from "../../http_requests/http_req";
...
const AuctionComp = () => {
  ...
  const fetchData = useCallback(async () => {
    const res = await get_all_services();
    ...
    >
      <div>
        ...
        >
          <span> Services</span>
          <span>
            Total Services : <span style={{ marginLeft: "10px" }}> {rows?.length} </span>
          </span>
        </div>
      </div>
      ...
      <div style={{ margin: "20px 50px", border: "1px solid white" }}>
        ...
        >
          <div style={{ marginRight: "10px", width: "33%" }}>Title</div>
          <div
