...
import { get_all_tickets } from "../../http_requests/http_req";
...
const AuctionComp = () => {
  ...
  const fetchData = useCallback(async () => {
    const res = await get_all_tickets();
    ...
    >
      <div>
        ...
        >
          <span> Tickets</span>
          <span>
            Total Tickets : <span style={{ marginLeft: "10px" }}> {rows?.length} </span>
          </span>
        </div>
      </div>
      ...
      <div style={{ margin: "20px 50px", border: "1px solid white" }}>
        ...
        >
          <div style={{ marginRight: "10px", width: "33%" }}>Subject</div>
          <div
